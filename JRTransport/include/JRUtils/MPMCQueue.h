#ifndef PROJECT_MPMCQUEUE_H
#define PROJECT_MPMCQUEUE_H
...
// NOTE: Implementation from Erik Rigtorp
...
class MPMCQueue {
  ...
  public:
    explicit MPMCQueue(const size_t capacity) : capacity_(capacity), head_(0), tail_(0)
    {
        ...
        void* buf = buf_;
        slots_ = reinterpret_cast<Slot*>(std::align(kCacheLineSize, capacity * sizeof(Slot), buf, space));
        if (slots_ == nullptr) {
            free(buf_);
            throw std::bad_alloc();
        }
