...
Our project is made of two subprojects : JRTransport and ProjB. ProjB depends of JRTransport.
The project produces 2 libraries (libJRTransport.so and libProjB.so) and 2 binaries linked against their
respective library (JRTransportrunner and projBrunner).
Packages for multiple platform should be produced : 1 single for ProjB and 3 different for JRTransport
(libs, devel, doc).
...
When compiling ProjB independently we must have installed JRTransport before, either from source or
from a package.
...
## Project structure and organisation
    ...
    │   │   └── JRTransportConfig.cmake    # for subprojects interdependencies
    │   ├── CMakeLists.txt           # main CmakeLists.txt for JRTransport
    │   ├── doc
    │   │   └── <Doxygen files and CMakeLists.txt>
    │   ├── include
    │   │   ├── CMakeLists.txt
    │   │   └── JRTransport                # project subfolder to appear in includes (e.g. include <JRTransport/World.h>)
    │   │       ├── CMakeLists.txt
...
the name of the project. We want that users of our libraries write "include <JRTransport/xxx.h>" and not
...
Important note : to be able to compile ProjB alone, one must first make and install JRTransport.
Moreover, one must add to PATH and LD_LIBRARY_PATH the installation directory of JRTransport.
